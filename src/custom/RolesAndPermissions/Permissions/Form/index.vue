  ...
  <a-card>
    <a-row type="flex">
      <a-col :span="12" :offset="2">
        <a-divider orientation="left">Форма разрешения</a-divider>
        <a-form-model
          :colon="false"
          ref="ruleForm"
          :model="form"
        ...
        >
          <a-row class="form-row" :gutter="16">
            ...
            <a-col>
              <a-form-model-item v-if="false" ref="name" label="Роль для разрешения" prop="user_type_id">
              ...
              <a-form-model-item label="">
                <a-row type="flex" justify="end">
                  <a-col span="auto">
                    ...
                    <a-button @click="postPermission" v-if="isNewForm" type="primary">{{ $t('button.create') }}</a-button>
...
export default {
  ...
  methods: {
    ...
    postPermission () {
      ...
      this.$store.dispatch('postPermission', this.form).then(res => {
        console.log(res)
    ...
    updatePermission () {
      ...
      this.$store.dispatch('updatePermission', this.form).then(res => {
        this.$alertMessage('success', 'Role', 'Role has updated successfully', this)
        this.cancelPost()
      })
    },
    cancelPost () {
      this.$router.replace('/role-permission/permissions')
    },
    handleSubmit (e) {
      console.log(e)
    }
