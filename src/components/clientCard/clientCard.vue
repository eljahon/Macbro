      ...
      <div style="display: flex;  flex: 0 0 23%;">
        <div class="two_card">
          <div style="display: flex;  align-items: center;  margin-left: 10px; gap: 5px">
            <img :src="img" alt="" />
            <span
            >{{ list.counter_agent.first_name }}{{ ' ' }}{{ list.counter_agent.last_name }}<br />
              <span style="color: #818C99; font-size: 16px">{{
                dataUseType.userType[list.counter_agent.user_type]
              }}</span></span
            >
          </div>
        </div>
        <div class="inside_wrapper">
          <div style="display: flex;  align-items: center;  margin-left: 10px; gap: 5px">
            <span
              style="color: red"
            >{{
               new Intl.NumberFormat('en-En', { style: 'currency', currency: 'USD' }).format(list.billing.total_amount)
             }}
              <br />
              <span
                style="color: #818C99; font-size: 16px; padding-right: 10px"
              >{{ summ(list.items).total }}/{{ summ(list.items).total_ammount }} Сканировано</span
              ></span
            >
          </div>
        </div>
