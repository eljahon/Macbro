    ...
    <a-card>
      ...
      >
        ...
        <template slot="Состояние" slot-scope="text, row">
          <table-user-column
            :image="row.causer.profile_image"
            :name="`${row.causer.first_name} ${row.causer.last_name}`"
            :phone="row.causer.phone_number"
            :type="row.causer.user_type"
          />
        </template>
        <template slot="Сумма" slot-scope="text, row">
          <span v-if="row">{{
            new Intl.NumberFormat('en-En', { style: 'currency', currency: 'USD' }).format(row.cash)
          }}</span>
        </template>
        <template slot="Статус" slot-scope="text, row">
          <a-tag :color="row.status === 'active' ? 'blue' : 'red'">{{
            row.status === 'active' ? 'Активный' : 'Не активный'
          }}</a-tag>
      ...
      <p class="comit_text">
        {{
          list.comment.length > 0
            ? list.comment
            : 'Lorem ipsum dolor sit amet, consectetur adipisicing elit. Asperiores atque commodi culpa cum cumque dolore est eum explicabo hic, nostrum possimus quam quasi quisquam repellendus, repudiandae sequi suscipit veritatis voluptatum?'
        }}
      </p>
      <br />
...
export default {
  ...
  methods: {
    ...
    getWritingListItem () {
      this.getWritingListItemId(this.$route.params.id).then(res => {
        this.list = res
