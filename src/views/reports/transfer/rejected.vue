<template>
  ...
  >
    ...
    <template slot="Статус" slot-scope="text, row">
      <a-tag :color="StatusSwitch(row.status).color"><span>{{ StatusSwitch(row.status).name }}</span></a-tag>
    </template>
  </a-table>
</template>
...
export default {
  computed: {
    ...mapGetters(['rejected', 'rejectedPagination']),
    getTabTwoList () {
      return this.rejected
    },
    getPaginationTwo () {
      return this.acceptedPagnation
    }
  },
  watch: {
  },
  data () {
    return {
      ...
      params: {
        ...
        statuses: 'rejected',
        // ,in-process,incomplete,rejected
        page: 1,
        limit: 10,
        number: '',
  ...
  methods: {
    ...mapActions(['getBranchList', 'AllTabListRejected']),
    moment,
    StatusSwitch (item) {
      switch (item) {
        case 'accepted' : return { color: 'blue', name: 'Принято' }
        case 'in-process' : return { color: 'Orange', name: 'В процессе' }
        case 'incomplete' : return { color: 'Orange', name: 'Неполный' }
      }
    },
    GetAllTabList () {
      this.loading = true
      this.AllTabListRejected(this.params)
        .then(res => {
        })
        .finally(() => {
          this.loading = false
