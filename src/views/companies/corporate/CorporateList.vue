<template>
  <div>
    ...
    <a-card :title="$t('juristicEntity')" class="breadcrumb-row" :bordered="false">
      <router-link :to="{ path: `${$route.path}/corporate/create` }" slot="extra">
    ...
    <a-card :bordered="false">
      ...
      <a-table
        ...
        :dataSource="getCompanyCorporatesList"
...
export default {
  ...
  computed: {
    ...mapGetters(['companyCorporatesList', 'companyCorporatesPagination', 'searchQuery']),
    getPagination () {
      return this.companyCorporatesPagination
    },
    getCompanyCorporatesList () {
      return this.companyCorporatesList
    },
    getSearchQuery () {
  ...
  mounted () {
      this.setSearchQuery()
    this.getCompanyCorporates({ page: this.companyCorporatesPagination, company_id: this.$route.params.id })
  ...
  methods: {
    ...mapActions(['getCompanyCorporates', 'setSearchQuery']),
    handleTableChange (pagination) {
      this.loading = true
      this.getCompanyCorporates({ page: pagination, search: true, company_id: this.$route.params.id })
    ...
    debouncedSearch (searchQuery) {
      ...
      this.loading = true
      this.getCompanyCorporates({ company_id: this.$route.params.id })
        .then((res) => console.log(res))
        .catch(err => this.requestFailed(err))
        .finally(() => (this.loading = false))
